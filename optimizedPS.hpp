#ifndef pointSignature_H
#define pointSignature_H
#include <time.h>
...
POINT_CLOUD_REGISTER_POINT_STRUCT(FSPoint,
								  (float, x, x)
								  (float, y, y)
								  (float, z, z)
								  (float, rgb, rgb)
...
public:
	...
	//Computes the siganture of the surface and saves it in 15 Degrees of interval
...
private:
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr iterationCloud, loopingCloud, filterCloud, outputCloud, planeCloud, filteredPlaneCloud;
	...
	*/
	...
	float mean, variance, standardDeviation, sum, upperLimit, lowerLimit, swap_var;
	
	//Flag for the counter True if inside, false if outside. Default = false
	bool isPlane, isSphere, isCylinder;

	inline void PointInPolygonTest(float* TXData, float* TYData, std::vector<temp> arrayinput, int nbK, int vertices, int &pointsInside);
	...
	satisfies the given condition and to store the squared distance of the same points*/
