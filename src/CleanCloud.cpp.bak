...
namespace PSA {
  ...
  float cleanCloud::calcMean( PCF::pointCloud input )
  {
    ...
    for( int i = 0; i < totalcount; i++ ) {
      if( kd_tree.nearestKSearch( input->points[ i ], _K, pointIdxNKNSearch,
    ...
    for( int i = 0; i < EuclidianDistance.size(); i++ ) {
      ...
      variance += ( EuclidianDistance[ i ] - meanDist ) *
                  ( EuclidianDistance[ i ] - meanDist );
    }
  ...
  PCF::pointCloud cleanCloud::removeNoise( PCF::pointCloud inputCloud )
  {
    ...
    std::vector< float > avgX( voxelCloud->points.size() ),
      avgY( voxelCloud->points.size() ), avgZ( voxelCloud->points.size() ),
      RGBData( voxelCloud->points.size() );
    ...
    for( int i = 0; i < voxelCloud->points.size(); i++ ) {
      if( octree.radiusSearch( voxelCloud->points[ i ], radius,
                               pointIdxRadiusSearch,
                               pointRadiusSquaredDistance ) > 0 ) {
        if( pointIdxRadiusSearch.size() >
            5 ) // Optimal values 4 when k = 6 or 7
        {
          for( size_t j = 0; j < pointIdxRadiusSearch.size(); j++ ) {
            // Accumulating the co-ordinates of the points who comes in the
            // volume sphere
            avgX[ counter ] = avgX[ counter ] +
                              voxelCloud->points[ pointIdxRadiusSearch[ j ] ].x;
            avgY[ counter ] = avgY[ counter ] +
                              voxelCloud->points[ pointIdxRadiusSearch[ j ] ].y;
            avgZ[ counter ] = avgZ[ counter ] +
                              voxelCloud->points[ pointIdxRadiusSearch[ j ] ].z;
            // RGBData[counter] = RGBData[counter] +
            // cleanSample->points[pointIdxRadiusSearch[j]].rgb;
          }
