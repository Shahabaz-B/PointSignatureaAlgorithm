...
float cleanCloud::calcMean(pcl::PointCloud<pcl::PointXYZ>::Ptr input) {
  ...
  for (int i = 0; i < EuclidianDistance.size(); i++) {
    ...
    variance +=
        (EuclidianDistance[i] - meanDist) * (EuclidianDistance[i] - meanDist);
  }
...
cleanCloud::removeNoise(pcl::PointCloud<pcl::PointXYZ>::Ptr inputCloud) {
  ...
  for (int i = 0; i < voxelCloud->points.size(); i++) {
    if (octree.radiusSearch(voxelCloud->points[i], radius, pointIdxRadiusSearch,
                            pointRadiusSquaredDistance) > 0) {
      if (pointIdxRadiusSearch.size() > 5) // Optimal values 4 when k = 6 or 7
      {
        for (size_t j = 0; j < pointIdxRadiusSearch.size(); j++) {
          // Accumulating the co-ordinates of the points who comes in the volume
          // sphere
          avgX[counter] =
              avgX[counter] + voxelCloud->points[pointIdxRadiusSearch[j]].x;
          avgY[counter] =
              avgY[counter] + voxelCloud->points[pointIdxRadiusSearch[j]].y;
          avgZ[counter] =
              avgZ[counter] + voxelCloud->points[pointIdxRadiusSearch[j]].z;
