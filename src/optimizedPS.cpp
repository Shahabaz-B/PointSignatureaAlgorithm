#include "optimizedPS.hpp"

pointSignature::pointSignature()
{
	cloud					= pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>);
	avgSearchRingCloudSize	= 0;
	loopingCloud			= pcl::PointCloud<pcl::PointXYZ>::Ptr (new pcl::PointCloud<pcl::PointXYZ>);
	planeCloud				= pcl::PointCloud<pcl::PointXYZ>::Ptr (new pcl::PointCloud<pcl::PointXYZ>);
	filteredPlaneCloud		= pcl::PointCloud<pcl::PointXYZ>::Ptr (new pcl::PointCloud<pcl::PointXYZ>);
	filterCloud				= pcl::PointCloud<pcl::PointXYZ>::Ptr (new pcl::PointCloud<pcl::PointXYZ>);
	outputCloud				= pcl::PointCloud<pcl::PointXYZ>::Ptr (new pcl::PointCloud<pcl::PointXYZ>);
	SearchRingCloud			= pcl::PointCloud<pcl::PointXYZ>::Ptr (new pcl::PointCloud<pcl::PointXYZ>);
...
void pointSignature::setRadius(float _radius)
{
	radius = _radius;
}

void pointSignature::setCenter(int _center)
{
...
void pointSignature::setMargin(float _margin)
{
	margin = _margin;
}

pcl::PointCloud<pcl::PointXYZ>::Ptr pointSignature::getCloud()
{
	return cloud;
}

float pointSignature::getRadius()
{
	return radius;
}

int pointSignature::getCenter()
{
	return center_point;
}

float pointSignature::getMargin()
{
	return margin;
}
...
void pointSignature::calculateSearchRing()
{
	...
	{
		//adding values for new points from the old point cloud which satisfy the condition
	...
	if (totalcount > 0)
	{
		...
		// 0.2617993878 = (15*2*PI)/360), standard forumula for arc length calculation
		float volume_sphere_radius = (0.2617993878 * (radius)); 
		for (size_t i = 0; i < totalcount; i++)
		{
			if (kdtree.radiusSearch(SearchRingCloud->points[i], volume_sphere_radius, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0)
			{
				for (size_t j = 0; j < pointIdxRadiusSearch.size(); j++)
				{
					//accumulating the co-ordinates of the points which comes in the volume sphere
					avgX[i] = avgX[i] + SearchRingCloud->points[pointIdxRadiusSearch[j]].x;
					avgY[i] = avgY[i] + SearchRingCloud->points[pointIdxRadiusSearch[j]].y;
					avgZ[i] = avgZ[i] + SearchRingCloud->points[pointIdxRadiusSearch[j]].z;
				}
...
void pointSignature::computeSignature()
{
	if (avgSearchRingCloudSize > 0)
	{
		...
		vcix.resize(avgSearchRingCloudSize);				//X co-ordiante of vector center and Intersecting point
		vciy.resize(avgSearchRingCloudSize);				//Y co-ordiante of vector center and Intersecting point
		vciz.resize(avgSearchRingCloudSize);				//Z co-ordiante of vector center and Intersecting point
		vciLength.resize(avgSearchRingCloudSize);			//length of the intersecting vector
		vciDotProd.resize(avgSearchRingCloudSize);		//Dot product between Normal vector of center point and intersecting vector
		vciLengthProd.resize(avgSearchRingCloudSize);		//length product between Normal vector of center point and intersecting vector
		cosTheta.resize(avgSearchRingCloudSize);			//cosTheta between Normal vector of center point and intersecting vector
		sFactor.resize(avgSearchRingCloudSize);			//scaling factor of the intersecting vector on the given Normal vector
		pNx.resize(avgSearchRingCloudSize);				//X co-ordinate of the point which is scaled on noraml vector
		pNy.resize(avgSearchRingCloudSize);				//Y co-ordinate of the point which is scaled on noraml vector
		pNz.resize(avgSearchRingCloudSize);				//Z co-ordinate of the point which is scaled on noraml vector
		vnix.resize(avgSearchRingCloudSize);				//X co-ordinate of the scaled noraml vector
		vniy.resize(avgSearchRingCloudSize);				//Y co-ordinate of the scaled noraml vector
		vniz.resize(avgSearchRingCloudSize);				//Z co-ordinate of the scaled noraml vector
		vniLength.resize(avgSearchRingCloudSize);			//Length of the vector between scaled point and the intersecting point
		cosThetaNi.resize(avgSearchRingCloudSize);		//cosTheta value for final plotting
		acosThetaNi.resize(avgSearchRingCloudSize);		//Angle for final plotting of profile, angle between each Vni vector w.r.t to reference Vni vector
		finalPlaneX.resize(avgSearchRingCloudSize);		//for extinding the angle value form 180 to 360 we need determinant of a value...
		finalPlaneY.resize(avgSearchRingCloudSize);		//which requires X and Y co-ordinate of the plane vector, which will be stored in these..
		...
		for(size_t i = 0; i < avgSearchRingCloudSize; i++ ) //loop for Vci Calculation
		{	
			...
			vciLength[i] = sqrt((vcix[i]*vcix[i])+(vciy[i]*vciy[i])+(vciz[i]*vciz[i]));
		}
		...
		for(size_t i = 0; i < avgSearchRingCloudSize; i++ )
		{
			...
			vciLengthProd[i] = (sqrt(pow(normalOfCenter.normal_x,2)+
				...
				pow(normalOfCenter.normal_z,2))) * vciLength[i];
		...
		//	for finding the cos angle between each Vni vector with respect to reference vector
		for(size_t i = 0; i < avgSearchRingCloudSize; i++ )
		{
			cosThetaNi[i] = ((vnix[i] * vnix[maxindex]) + (vniy[i] * vniy[maxindex]) + (vniz[i] * vniz[maxindex]))/
				((vniLength[i])*(vniLength[maxindex])); 	

			//	due to rounding error the value of cosThetaNi might go higer than 1 or lower than -1 so we need this inequality for getting correct value of angle
			if((( (cosThetaNi[i] - 1 <=0.00001) && (cosThetaNi[i] - 1 >=0.0) ) || ( (cosThetaNi[i] + 1 <= 0.00001) && (cosThetaNi[i] + 1 >= 0.0 ) )))
			{
				acosThetaNi[i] = 0;
			}
			else
			...
			//calculating the X and Y co-ordinate of the scaled circle of intersectin points
		...
		for(size_t i = 0; i < avgSearchRingCloudSize; i++)
		{
			...
			value[i] =  ((finalPlaneX[maxindex] - center.x) * (finalPlaneY[i] - center.y)) -
				((finalPlaneY[maxindex] - center.y) * (finalPlaneX[i] - center.x));
		}

		//		assigning correct value for final Theta (extending the anlge form 0-180 to 0-360)
		for(size_t i = 0; i < avgSearchRingCloudSize; i++)
		{
			if(value[i]<0)
			{
...
void pointSignature::PointInPolygonTest(float* TXData, float* TYData, std::vector<temp> arrayinput, int nbK, int vertices, int &pointsInside)
{
	pointsInside = 0;
	bool isInside = false;
	for (int k = 0; k < nbK; k++ )
	{
		...
		{
			if(((TYData[i] >= arrayinput[k].scaling) != (TYData[j] >= arrayinput[k].scaling))&&
				(arrayinput[k].theta <= ((TXData[j] - TXData[i])/(TYData[j] - TYData[i])) * (arrayinput[k].scaling - TYData[i]) + TXData[i]))
			{
				isInside = !isInside;
			}
		}
...
void pointSignature::SigCompare()
{
	...
	//=======================start of checking if clicked point is plane or not=======================================
	...
	if((mean > -0.2) && (mean < 0.4))
	{
		...
		PointInPolygonTest(planeTXData,planeTYData,normArray,25,vertices,planarPoints);
	...
	else if ((mean > 0.8) && (mean < 1.0))
	{
		...
		PointInPolygonTest(sphereTXData,sphereTYData,normArray,25,vertices,spherePoints);
	...
	else if ((mean > 0.4) && (mean < 0.7))
	{
		...
		float cylinderTYData[16] = {1.25, 0.36646, 0.197709, 1.18, 1.2, 0.23, 0.186678, 1.25,
		...
		PointInPolygonTest(cylinderTXData,cylinderTYData,normArray,25,vertices,cylinderPoints);
...
void pointSignature::checkShape()
{
	...
	for (int i = 0; i < totalpoints; i++)
	{
		...
		if(avgSearchRingCloudSize > 10)
		{
			...
			if( (shape == pcl::SACMODEL_CYLINDER && isCylinder == true || 
				...
				( kdtree.radiusSearch (cloud->points[i], radius, pointIdxRadSearch, pointRadSqDistance) > 10 ))
			{
	...
	{
		size = (outputCloud->points.size() + 0.35 * filteredPlaneCloud->points.size());
		printf("Cone, size = %d\n", size);
	}
