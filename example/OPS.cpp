...
int main(int argc, char** argv)
{
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr extractedCloud (new pcl::PointCloud<pcl::PointXYZ>);
	pcl::visualization::PCLVisualizer *visualizer (new pcl::visualization::PCLVisualizer);
	...
	pcl::transformPointCloud(*extractedCloud, *cloudPointsProjected, projectionTransform);
...
//	cleanCloud cc;
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr SearchRingCloud (new pcl::PointCloud<pcl::PointXYZ>);
	...
	for (size_t i = 0; i < SearchRingCloud->points.size(); i++)
	{
		...
		SearchRingCloud->points[i].x = cloudPointsProjected->points[ptIndex[i]].x;
		SearchRingCloud->points[i].y = cloudPointsProjected->points[ptIndex[i]].y;
		SearchRingCloud->points[i].z = cloudPointsProjected->points[ptIndex[i]].z;
	}
	...
	std::vector<float>	vcix, vciy, vciz, vciLength,
		vciDotProd, vciLengthProd, cosTheta,
		sFactor, pNx, pNy, pNz, vnix, vniy, vniz, vniLength,
		cosThetaNi, acosThetaNi, finalPlaneX, finalPlaneY, value;
	...
	{	
		...
		vniLength[i] = sqrt((vnix[i]*vnix[i])+(vniy[i]*vniy[i])+(vniz[i]*vniz[i]));
	}
	...
	for(int i = 0; i < SearchRingCloud->points.size(); i++)
		var[i] = (SearchRingCloud->points[i].x - mid) * (SearchRingCloud->points[i].x - mid);
	...
	{
		//if(var[i] > 0.0012)
		fileData << SearchRingCloud->points[i].x << "\t" << SearchRingCloud->points[i].y << "\t" << SearchRingCloud->points[i].z << "\t" << SearchRingCloud->points[i].x << "\t" << acosThetaNi[i] << "\t" << var[i] << "\n";
	}
	fileData.close();
	///=================================================================
